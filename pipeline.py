...
The `optimize` step (SIMBA) has two standalone alternatives, not part of this
linear pipeline: `uv run optimize-gepa` and
`uv run steps/optimize_prompts_textgrad.py`. Compare their results with
`uv run compare-optimizers`. See EXPERIMENT.md.
"""
