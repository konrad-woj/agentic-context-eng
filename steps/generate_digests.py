...
# Which optimizer's output to load — one of: simba | gepa | textgrad.
# Defaults to "simba" for exact backward compatibility with prior behavior.
# Uses `or` (not the two-arg os.environ.get default) so a blank
# PROMPT_SOURCE= in .env falls back correctly instead of resolving to "".
PROMPT_SOURCE = (os.environ.get("PROMPT_SOURCE") or "simba").lower()
_SOURCE_FILENAMES = {
    "simba":    "extractor_optimized.json",
    "gepa":     "extractor_optimized_gepa.json",
    "textgrad": "extractor_optimized_textgrad.json",
}
...
def load_optimized_prompt(persona: str) -> tuple[str, list[dict], bool]:
    """
    Loads the optimized prompt selected by PROMPT_SOURCE (simba/gepa/textgrad)
    from artifacts/prompts/.

    Returns (system_prompt, few_shot_demos, is_optimized).
    Falls back to the hardcoded prompt if the requested file does not exist.

    Two-layer architecture:
      - SIMBA/GEPA/TextGrad optimize the domain extractor (instructions + few-shot)
      - Persona-specific formatting is added on top
    """
    filename = _SOURCE_FILENAMES.get(PROMPT_SOURCE, _SOURCE_FILENAMES["simba"])
    opt_path = PROMPTS_DIR / filename

    if not opt_path.exists():
        if PROMPT_SOURCE != "simba":
            print(f"  ⚠  PROMPT_SOURCE={PROMPT_SOURCE!r} requested but {filename} "
                  f"not found — using hardcoded fallback")
...
def generate_digest(snapshot: dict, persona: str) -> dict:
    ...
    return {
        ...
        "persona":       persona,
        "model":         MODEL,
        "prompt_source": f"{PROMPT_SOURCE}_optimized" if is_optimized else "fallback_hardcoded",
        "raw_output":    raw,
        "cards":         blocks,
...
def main():
    ...
    opt_path = PROMPTS_DIR / _SOURCE_FILENAMES.get(PROMPT_SOURCE, _SOURCE_FILENAMES["simba"])
    if opt_path.exists():
        opt   = json.loads(opt_path.read_text())
        score = opt.get("optimization_log", {}).get("optimized_score", "?")
        print(f"  Prompt: {PROMPT_SOURCE}-optimized (score={score})")
    else:
        print(f"  Prompt: fallback hardcoded")
        print(f"  Tip: run uv run optimize (or optimize-gepa / "
              f"steps/optimize_prompts_textgrad.py + PROMPT_SOURCE=...) "
              f"to use an optimized prompt")
    print()
