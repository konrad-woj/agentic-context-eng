...
SIMBA isn't the only way to do this, and we didn't want to just assert that it's the right choice. We ran the same extraction task through two alternatives on the identical trainset and metric: GEPA, DSPy's newer reflective/evolutionary optimizer, and TextGrad, which backpropagates textual feedback through the prompt as a differentiable variable instead of sampling mini-batches. We're not crowning a winner in this post — which one wins depends on your dataset size and your reflection-model budget — but the full head-to-head, including how to reproduce it, is in `EXPERIMENT.md`.
...
```bash
uv run collect          # arXiv + HN + RSS, no API key needed
uv run gen-trainset     # synthetic labeled examples via Gemini Flash
uv run optimize         # SIMBA optimizes the domain extractor
uv run gen-digests      # generate REPLICATOR CARDs and DECISION BRIEFs
uv run rate             # human expert rates each card 1–5
uv run score            # derive ACE playbook entries from ratings
```
...
Required: `uv sync` (installs all dependencies from `pyproject.toml`)
Required: `GEMINI_API_KEY` in `.env` (or swap to any LiteLLM-supported provider — see `.env.example`)
