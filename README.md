...
steps/        — pipeline step scripts (collect, train, optimize, digest, rate, score)
  optimize_prompts.py           — SIMBA optimizer (default, uv run optimize)
  optimize_prompts_gepa.py      — GEPA optimizer (alternative, uv run optimize-gepa)
  optimize_prompts_textgrad.py  — TextGrad optimizer (alternative, standalone PEP 723 script)
  compare_optimizers.py         — compares SIMBA/GEPA/TextGrad results (uv run compare-optimizers)
lib/          — shared library code
  extraction_metric.py — canonical scoring rubric used by every optimizer above
  ace_two_layer.py, mipro_ace_pipeline.py — ACE reference architecture (see "ACE library" below)
artifacts/    — all generated data (gitignored)
  data/       — collected AI news snapshots
  trainset/   — SIMBA/GEPA/TextGrad training examples + review report
  prompts/    — optimized extraction prompts (one file per optimizer) + optimizer_comparison.md
...
EXPERIMENT.md — log of alternative approaches tried on this branch (optimizers, ACE architecture)
...
## Alternative prompt optimizers

SIMBA is the pipeline default, but two alternatives are available for
comparison — see `EXPERIMENT.md` for the full writeup:

```bash
uv run optimize                              # SIMBA (default)
uv run optimize-gepa                         # GEPA — reflective/evolutionary (requires dspy>=3.0)
uv run steps/optimize_prompts_textgrad.py    # TextGrad — textual backpropagation
uv run compare-optimizers                    # compares whichever of the above have been run
```

All three read the same `artifacts/trainset/trainset_domain.json` and are
scored by the same metric (`lib/extraction_metric.py`), so their results are
directly comparable. `gen-digests` defaults to SIMBA's output; set
`PROMPT_SOURCE=gepa` or `PROMPT_SOURCE=textgrad` to use a different
optimizer's result instead.
...
`optimize-gepa` and `optimize_prompts_textgrad.py` additionally use `REFLECTION_MODEL` for their reflection/gradient-generation step (defaults to a stronger tier of the same provider — e.g. `gemini-2.5-pro` when `LLM_PROVIDER=gemini`). `optimize-gepa` also reads `GEPA_AUTO` (`light`/`medium`/`heavy`, default `light`) as a cost/quality dial.

For MLX, start the inference server first (OpenAI-compatible, expected on port 8888):
